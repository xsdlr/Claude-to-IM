 ...
 *   BRIDGE_MODEL=claude-sonnet-4-20250514           (default: demo-echo)
 */
...
function buildSettings(): Record<string, string> {
  const settings: Record<string, string> = {
    ...
    bridge_default_cwd: process.env.BRIDGE_DEFAULT_CWD?.trim() || process.cwd(),
    bridge_default_model: process.env.BRIDGE_MODEL?.trim() || 'demo-echo',
    default_model: process.env.BRIDGE_MODEL?.trim() || 'demo-echo',
  };
...
async function main() {
  ...
  initBridgeContext({
    store,
    llm: new EchoLLM({ markdown: true, label: 'DingTalk Stream Demo' }),
  ...
  console.log('');
  console.log('Demo is running. Send a message to the DingTalk bot to receive an Echo response.');
